...
from ..models import Order, OrderStatus, Payout, PayoutStatus, Photo, PhotoStatus, WorkType


def recalc_payout_for_order(db: Session, order: Order):
    photos = db.query(Photo, WorkType).join(WorkType, WorkType.id == Photo.work_type_id).all()
    order_item_ids = {item.id for item in order.items}
    preliminary = 0.0
    final = 0.0
    for photo, wt in photos:
        if photo.order_item_id in order_item_ids:
